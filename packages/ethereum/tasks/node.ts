...
import { JsonRpcServer as JsonRpcServerImpl, JsonRpcServerConfig } from '../internal/hardhat-network/jsonrpc/server'
import { Reporter } from '../internal/sentry/reporter'
import { EthereumProvider, HardhatNetworkConfig, JsonRpcServer } from '../types'
...
function printDefaultConfigWarning() {
  console.log(chalk.bold('WARNING: These accounts, and their private keys, are publicly known.'))
  console.log(chalk.bold('Any funds sent to them on Mainnet or any other live network WILL BE LOST.'))
}

function logHardhatNetworkAccounts(networkConfig: HardhatNetworkConfig) {
  const isDefaultConfig =
    !Array.isArray(networkConfig.accounts) && networkConfig.accounts.mnemonic === HARDHAT_NETWORK_MNEMONIC

  const { BN, bufferToHex, privateToAddress, toBuffer } = require('ethereumjs-util') as typeof EthereumjsUtilT
...
subtask(TASK_NODE_GET_PROVIDER)
  ...
  .setAction(
    async (
      ...
      // FORK NOTE: Because this task only be called with hardhat networks, we can
      // safely ignore trying to set up a provider object.
      const hardhatNetworkConfig = config.networks[network.name]
...
subtask(TASK_NODE_SERVER_READY)
  ...
  .setAction(
    async (
      ...
      console.log(chalk.green(`Started HTTP and WebSocket JSON-RPC server at http://${address}:${port}/`))
...
task(TASK_NODE, 'Starts a JSON-RPC server on top of Hardhat Network')
  ...
  .addOptionalParam('port', 'The port on which to listen for new connections', 8545, types.int)
  .addOptionalParam('fork', 'The URL of the JSON-RPC server to fork from', undefined, types.string)
  .addOptionalParam('forkBlockNumber', 'The block number to fork from', undefined, types.int)
  .setAction(
    async (
      {
        forkBlockNumber,
      ...
      // FORK NOTE: Instead of checking for name equality we check for hardhat type
      // equality.
      if (!network.isHardhat) {
