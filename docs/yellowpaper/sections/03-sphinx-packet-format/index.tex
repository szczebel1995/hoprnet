...
We define $r$ to be the maximum number of nodes that a Sphinx packet will traverse before arriving to its destination.
...
\end{align}
where $\phi_0=\epsilon$ is an empty string. $\phi_i$ is generated using the shared secret $\varphi_{i-1}$ and used to ensure the header packets remain constant in size. Upon receiving a packet, the node extracts the routing information destined to it and decryptes one layer of the payload before sending to the next downstream node. The extraction is performed by deobfuscating and left-shifting the field. Ordinarily, this would make the field shorter at each hop, allowing an attacker to deduce the route length. For this reason, the field is pre-padded before forwarding.
